...
core.workflow_chat(채팅 생성/수정)과 core.workflow_modifier(전용 수정 엔드포인트)가 공유한다.
pass-through 날조 차단 자체는 hydrate_node가 하고(원본 dict를 넘긴 호출부만 열린다),
이 모듈은 그 원본 dict를 구성해 넘기는 역할이다."""
import re
...
def prepare_hydrated_nodes(
    ...
    *,
    ...
    pass-through draft(templateId=PASSTHROUGH_TEMPLATE_ID)는 current_nodes의 서버 측 원본에서만
    복원된다 — LLM이 보낸 "node"는 hydrate_node가 무시한다(날조로 슬롯 검증을 우회하는 것 차단).
    current_nodes에 없는 id면 SlotFillError(신규 노드를 pass-through로 날조하는 것 차단).
    ...
    current_nodes_by_id = {
        n.get("id"): n for n in (current_nodes or []) if isinstance(n, dict) and n.get("id")
    }
