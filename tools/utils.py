...
def json_parse(
    ...
    Note:
        대량의 로우 JSON 데이터(목록 등)를 분류·요약할 때 사용한다. 추출한 값을 최종 응답에
        반영할 때는 인사말·서론 없이 정제된 결과만 즉각적으로 반환한다.
    """
    try:
...
def text_extract(
    ...
    Note:
        ...
        find_all=True로 여러 항목을 추출했다면 최종 응답에서 목록 형태로 구조화하고,
        불필요한 서론 없이 정제된 결과만 반환한다.
    """
    try:
...
def date_format(
    ...
    Note:
        마크다운 보고서(예: 주간 PR 요약)에 날짜를 표기할 때 이 도구로 형식을 통일한 뒤,
        인사말이나 서론 없이 구조화된 결과물에 바로 반영한다.
    """
